...
#         for radial_flag in [True, False]:
#             amp, phase, aperture = self.get_lens(radial_symmetry=radial_flag)
#             amp = torch.tensor(amp[None], dtype=torch.float32).to(device=device)
#             phase = torch.tensor(phase[None], dtype=torch.float32).to(device=device)
#             wavelength_set_m = [400e-9, 600e-9]
...
#             assert (
#                 asm_amp.shape == asm_phase.shape == fres_amp.shape == fres_phase.shape
#             )
...
#             if not os.path.exists(out_dir):
#                 os.makedirs(out_dir)
#             plot_path = os.path.join(out_dir, f"prop_radial_{radial_flag}_{device}.png")
#             plt.savefig(plot_path)
#             plt.close()
