...
def test_reverse_lookup_optimize_real_execution(model_name, pol_dim):
    ...
    assert op_param.shape == (B, H, W, results[0].shape[-1]), "Output parameter shape mismatch"
    assert len(err_list) == 3, "Should have exactly 3 entries in error list"
    print(f"Test completed for model {model_name} with polarization dimension {pol_dim}.")
