...
def get_detector_pixel_coordinates(propagation_parameters):
    ...
    x = (
        np.arange(0, sensor_pixel_number["x"], 1) - (sensor_pixel_number["x"] - 1) / 2
    ) * sensor_pixel_size_m["x"]
    y = (
        np.arange(0, sensor_pixel_number["y"], 1) - (sensor_pixel_number["y"] - 1) / 2
    ) * sensor_pixel_size_m["y"]
    return x, y
...
def get_lens_pixel_coordinates(propagation_parameters):
    ...
    if radial_symmetry:
        ...
        x = (np.arange(0, ms_samplesM["x"], 1) - (ms_samplesM["x"] - 1) / 2) * ms_dx_m[
            "x"
        ]
        y = (np.arange(0, ms_samplesM["y"], 1) - (ms_samplesM["y"] - 1) / 2) * ms_dx_m[
            "y"
        ]
        return x, y
