...
def radial_2d_transform(r_array):
    ...
    xx, yy = torch.meshgrid(torch.arange(1 - N, N), torch.arange(1 - N, N), indexing="ij")
...
def interp_regular_1d_grid(x_min, x_max, y_ref, xi):
    ...
    """
    # Input tensors checked in tf_generalSpline_regular1DGrid so we won't check it again
...
def helper_spline_complex(r_ref_min, r_ref_max, r, fr):
    ...
    torch_zero = torch.tensor(0.0, dtype=f_transform_abs.dtype).to(fr.device)
...
def tf_generalSpline_regular1DGrid(r_ref, r, fr):
    ...
    if not torch.is_tensor(r_ref):
        r_ref = torch.tensor(r_ref, dtype=dtype).to(device)
