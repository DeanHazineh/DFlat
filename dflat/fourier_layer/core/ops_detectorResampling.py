...
def resize_area(image, resize_ratio):
    ...
    rectFilter = torch.ones((1, 1, resize_ratio[0], resize_ratio[1]), dtype=image.dtype).to(image.device)
...
def sensorMeasurement_intensity_phase(sensor_intensity, sensor_phase, parameters, use_radial):
    ...
    tol = 0.1
    diff_x = abs(sensor_pixel_size_um["x"] - calc_sensor_dx_um["x"])
    diff_y = abs(sensor_pixel_size_um["y"] - calc_sensor_dx_um["y"])
    ...
    if diff_x > tol or diff_y > tol:
        ...
        scalex = sensor_pixel_size_um["x"] / calc_sensor_dx_um["x"]
        scaley = sensor_pixel_size_um["y"] / calc_sensor_dx_um["y"]
        upsample_size = (
