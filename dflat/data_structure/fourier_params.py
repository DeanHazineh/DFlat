...
class prop_params(dict):
    ...
    def __torch_add(self):
        # Updated code to propagation parameters for pytorch port
        dtype = self.__dict__["dtype"]
        self.__dict__["torch_zero"] = torch.tensor(0.0, dtype=dtype)
    ...
    def __add_implied_keys(self):
        ...
        ms_samplesM = self.__dict__["ms_samplesM"]
        ms_length_m = {
            "x": ms_dx_m["x"] * ms_samplesM["x"],
            "y": ms_dx_m["y"] * ms_samplesM["y"],
        }
    ...
    def __enforce_odd_sensor_pixel_number(self):
        ...
        self.__dict__["sensor_pixel_number"] = {
            "x": sensor_num_x,
            "y": sensor_num_y,
            "r": sensor_num_r,
        }
    ...
    def __regularizeLensPlaneSampling(self):
        ...
        if automatic_upsample:
            ...
            calc_ms_dx = ms_dx_m["x"] / manual_upsample_factor if manual_upsample_factor > 1.0 else ms_dx_m["x"]
            calc_ms_dy = ms_dx_m["y"] / manual_upsample_factor if manual_upsample_factor > 1.0 else ms_dx_m["y"]
